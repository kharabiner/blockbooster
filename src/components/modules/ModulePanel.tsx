...
export type ModulePanelProps = {
  ...
  isOperator?: boolean; // 부스 운영자인지 여부
};

export function ModulePanel({ slotId, moduleId, config, isOperator }: ModulePanelProps) {
  const Panel = MODULE_PANELS[moduleId];
  if (!Panel) return null;
  return (
    <Suspense fallback={<Skeleton className="h-24 w-full rounded-lg" />}>
      <Panel slotId={slotId} moduleId={moduleId} config={config} isOperator={isOperator} />
    </Suspense>
  );
}
